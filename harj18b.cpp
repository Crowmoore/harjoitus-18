...
int compareDogNames(const void*, const void*);
int getDogCount();
...
int main()
{
	...
	qsort(dogs, numberOfDogs, sizeof(dogInfo), compareDogNames);
	clearConsole();
...
int compareDogNames(const void * a, const void * b)
{
	if (((const dogInfo*)a)->dogName < ((const dogInfo*)b)->dogName) return -1;
	if (((const dogInfo*)a)->dogName == ((const dogInfo*)b)->dogName) return 0;
	if (((const dogInfo*)a)->dogName >((const dogInfo*)b)->dogName) return 1;
}